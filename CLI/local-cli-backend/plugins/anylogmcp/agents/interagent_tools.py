...
from plugins.anylogmcp.agents.visualization.charts.charting_agent import PlotChartRequest
from plugins.anylogmcp.agents.visualization.tables.tabular_agent import GenerateTableRequest
...
def setup_interagent_tools(agent: "AnylogAgent"):
    @agent.tool
    async def plot_charts(ctx: RunContext[AnylogAgentDeps], charts: List[PlotChartRequest]) -> str:
        """
        ...
        # await ctx.deps.resultFn("Visualizing your data...", StreamingMarker.STATUS_UPDATE)
        results = []
        failed_charts = []
        
        for idx, plot in enumerate(charts):
            print(f"Plotting: {plot.plot_title}")
            await ctx.deps.resultFn(f"Plotting: {plot.plot_title}", StreamingMarker.STATUS_UPDATE)
            try:
                chart = await ctx.deps.chart_agent.generate_chart(
                    f'Create me a chart of this: {plot}'
                )
        ...
        if not failed_charts:
            return "SUCCESS: All charts successfully plotted. Continue with your response."
        else:
            failure_details = "\n".join([f"- Chart #{f['index']}: {f['error']}" for f in failed_charts])
            return (
                f"PARTIAL SUCCESS: {len(charts) - len(failed_charts)}/{len(charts)} charts succeeded.\nFailed charts:\n{failure_details}\n\nPlease retry ONLY the failed charts with corrected data or different parameters."
            )

    @agent.tool
    async def generate_table(ctx: RunContext[AnylogAgentDeps], table_request: GenerateTableRequest) -> str:
        """
        Generate a table. Returns status of table attempt.
        """
        print(f"Constructing: {table_request.table_title}")
        await ctx.deps.resultFn(f"Constructing: {table_request.table_title}", StreamingMarker.STATUS_UPDATE)
        ...
        try:
            ...
            print(table_result)
            await ctx.deps.resultFn(table_result.model_dump(), StreamingMarker.TABLE_DATA)
    ...
    async def perform_task(ctx: RunContext[AnylogAgentDeps], task: str) -> str:
        ...
        print(f"Task: {task}")
        await ctx.deps.resultFn(f"Contacting the AnyLog network", StreamingMarker.STATUS_UPDATE)
        
        try:
            task_result = await ctx.deps.mcp_agent.perform_task(
                f'Complete this MCP task: {task}'
            )
